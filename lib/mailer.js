...
// Anexa public/email-logo.png (se existir) e devolve o cid para o HTML.
function lerLogo() {
  try {
...
export function htmlNotificacao({ nome, email, assunto, mensagem }) {
  const corpo = `
...
export async function enviarEmailContacto({ nome, email, assunto, mensagem }) {
  await getTransporter().sendMail({
    ...
    html: htmlNotificacao({ nome, email, assunto, mensagem }),
  });
}
