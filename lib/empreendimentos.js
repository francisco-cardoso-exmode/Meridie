...
// Código do país para a referência.
const PAIS_COD = { portugal: "PT", brasil: "BR" };

// Abreviatura (3 letras) da cidade — sem acentos nem espaços.
const cidadeCod = (cidade) =>
  (cidade || "")
    .normalize("NFD")
...
// Referência (código identificador) do empreendimento. Usa a definida no
// backoffice; se vazia, gera uma estável no formato PAÍS-CIDADE-NÚMERO
// (ex.: PT-LIS-042, BR-FOR-118). O número é estável (derivado do imóvel), para
// a referência ser permanente — não muda ao apagar/adicionar outros imóveis.
export function referenciaDe(e) {
  if (!e) return "";
  if (e.referencia) return e.referencia;
  const slug = e.slug || "";
  let n = 0;
  for (let i = 0; i < slug.length; i++) n = (n * 31 + slug.charCodeAt(i)) >>> 0;
  const num = String((n % 999) + 1).padStart(3, "0");
  return `${PAIS_COD[e.pais] || "XX"}-${cidadeCod(e.cidade)}-${num}`;
}
